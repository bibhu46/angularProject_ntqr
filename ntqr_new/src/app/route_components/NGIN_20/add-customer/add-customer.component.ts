...
export class AddCustomerComponentV20 implements OnInit {
  customerList: any;
  ...
  ngOnInit(): void {
    this.titleService.setTitle('NGIN 20 Add Customer');  
  ...
  onCancel() {
    this.router.navigate(['/ngin20/customerReports']);
  }

  addCustomer() {
    this.router.navigate(['/ngin20/addCustomer']);
  }
  ...
  editCustomer(name, id) {
    this.router.navigate(['/ngin20/editCustomer/' + name +"/"+ id]);
  }
}
