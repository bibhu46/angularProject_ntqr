...
export class ServiceKeysComponentV20 implements OnInit {
  ...
  ngOnInit(): void {
    this.titleService.setTitle('NGIN 20 Service Keys');
  ...
  onEdit(event: MouseEvent, countryId: number) {
    this.route.navigateByUrl('ngin20/countryGroupEdit/' + countryId);
  }
  ...
  add_serviceKey() {
    this.route.navigateByUrl('/ngin20/addServiceKey');
  }

  serviceKeyDetails(event: MouseEvent, countryId: number) {
    console.log('called', countryId);
    this.route.navigateByUrl('ngin20/viewServiceKey/' + countryId);
  }
  array_toString(arr) {
    return arr.join(', ');
  }
