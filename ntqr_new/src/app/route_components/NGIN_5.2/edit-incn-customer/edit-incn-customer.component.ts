...
export class EditIncnCustomerComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.titleService.setTitle('NGIN 5.2 Edit Customer'); 
  ...
  customer_submit_form() {
    ...
    this.CustomerReportsService.editCustomerInList(submit_obj).subscribe((results) => {
      ...
      this.router.navigate(['/ngin5.2/addedCustomerList']);
    })
  }
