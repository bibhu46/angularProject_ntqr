...
export class InstalledNumbComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.titleService.setTitle('NGIN 5.2 Installed Numbers'); 
  ...
  showPopUpHistory(countryId: string, desc: string, name: string) {
    ...
    let url = appConfig.hostUrl + "ngin5.2/installedNumbersPopup/" + countryId + "/" + desc + "/" + name + "/" + this.date;
  ...
  showDetails(countryId: string, desc: string, name: string) {
    ...
    let url = "ngin5.2/openServiceKey/" + countryId + "/" + desc + "/" + name + "/" + date_new;
    console.log(url)
