...
export class ViewServiceKeyComponent implements OnInit {
  countryId: string;
  range: string;
  key: string;
  type: string;
  ...
  onDelete() {
    ...
    this.ServiceKeyService.removeServiceKey(this.countryId).subscribe((results) => {
      ...
      this.route.navigate(['/ngin5.2/serviceKeys']);
    })
  }

  onEdit() {
    this.route.navigateByUrl('/ngin5.2/updateServiceKey/'+ this.countryId);
  }

  onCancel() {
    this.route.navigateByUrl('/ngin5.2/serviceKeys');
  }
