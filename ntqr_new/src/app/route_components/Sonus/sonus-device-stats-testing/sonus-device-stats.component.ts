...
@Component({
  selector: 'app-sonus-device-stats',
  templateUrl: './sonus-device-stats.component.html',
  styleUrls: ['./sonus-device-stats.component.scss']
})
export class SonusDeviceStatsComponent implements OnInit {
  ...
  constructor(private api: DeviceKpiService, private global: GlobalService) { }
  ...
  getAll(newVal) {
    this.api.getAll(newVal).subscribe((results) =>  {
      ...
      if(this.statGrp[0] == "DSP Card Usage"){
        ...
        this.slotSel = this.slot[0]['label'];
        this.selDevTp = [];
  ...
  onSubmit(userForm) {
    ...
    deviceTypes = this.dev_type_sel;
    devices = _.pluck(userForm.selDevTp, "label");

    if(userForm.statGrp[0] == 'System Health Stats' && (deviceTypes.includes('SBC') || deviceTypes.includes('DSI')) && deviceTypes.length==1){
      sbcCon=true;
    }
    
    obj = new Obj(fromDate,toDate,userForm.statGrp[0],deviceTypes,devices,userForm.slot,'');    
...
onClickKpi() {
  ...
  document.getElementById('kpi2').style.display = '';
  document.getElementById('save').style.display = '';  
}
save(id){
  let note=(<HTMLInputElement>document.getElementById(id)).value;
  document.getElementById('edit').style.display = '';
  document.getElementById('kpi1').style.display = '';
...
showPopUpHistory(deviceName,deviceType,shelf,slot,path,cpuCount,cpuName,descr,codecName,resourceReport,nifPort) {
  ...
  let url = "http://localhost:4200/ntqrGUI/"+"devicekpi/popup/systemHealthStats";
...
on_devices_select(ev){
  console.log("Devices on select data");
  console.log(this.selDevTp);  

  if(this.selDevTp.length > 1 && (this.statGrp[0] == "DSP Card Usage" || this.statGrp[0] == "Ethernet Packet Port Status")){    
    this.selDevTp.pop();     
    
    let mess = "Only one device must be selected for DSP Card Usage";

    let modal_obj = {
      title: 'Device <span>Stats </span> Error',
      size: "sm",
      color: "green",                
      auto_close: false,
...
device_type_change(ev){
  ...
  console.log("selDevType");
  console.log(this.selDevType);
  console.log(this.filter_form_value['dev_types']);
